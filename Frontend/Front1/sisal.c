...
static void ReadTableInt( FILE*, int* );
...
/* p2c emitted a "Static" storage-class shim here; sisalInfo.h defined it as
   static and this file fell back to nothing.  Both are gone -- declarations
   just say static. */
...
/* -----------------------------------------------
 ...
 #        ####   #    #  #    #  #    #  #    #  #####   
 ----------------------------------------------- */
static void    PASCAL_MAIN (int, Char **) ;
static Char    **P_argv;
static int     P_argc;
static SHORT   P_escapecode;
static int     P_ioresult;
static void CANN_exit(int);


/* static Anyptr   malloc (size_t) ; */
/* static void     free (Anyptr) ; */

/* static int      _OutMem(void); */
static Anyptr   _OutMem(void);

static int      _Escape (int) ;
static int      _EscIO (int) ;

static int      P_peek (FILE *) ;
static int      P_eof (FILE *) ;
static int      P_eoln (FILE *) ;
static int      P_inset (unsigned, long *) ;
static long    *P_addset (long *, unsigned) ;
static long    *P_addsetr (long *, unsigned, unsigned) ;
static long    *P_remset (long *, unsigned) ;
static long    *P_expset (long *, long) ;
static void     P_sun_argv (char *, int, int) ;

static node* buildcompound_( lexleveltype );
static void addgrptr( graph*,node* );
static node* buildsubgraph( node* );
static void insertat( int );
static void addtolist_( int,int );
static void addtoxref( symtblbucket* );
static void printxref( void );
static void endlevel( void );
static void endcompound( void );
static void addtoassolist( node*,int );
static port* orderolist( port* );
static port* orderilist( port* );
static void createlevel( lexleveltype );
static symtblbucket* createbucket( identtype );
static void setname( symtblbucket*,stryng );
static void setdefloc( symtblbucket*,int,int );
static void addref( symtblbucket*,int,int );
static void setlowerbound( treestackobj*,inforecord* );
static int getdefline( symtblbucket*,int );
static int getdefcolumn( symtblbucket*,int );
static loopvarflags getloopflag( symtblbucket* );
static int hash( Char,Char );
static void addtotable( symtblbucket*,treestackobj* );
static symtblbucket* incurrentlevel( stryng,identtype );
static void findname( stryng,identtype,treestackobj**,symtblbucket** );
static void addpredefined( stryng,predeftype );
static void initsymtable( void );
static void outtype( FILE*,stentry*,boolean );
static void printbucket( symtblbucket* );
static void printtable( FILE*,treestackobj* );
static void printtypes( FILE* );
static void printtree( FILE*,treestackobj* );
static void crossref( FILE*,treestackobj* );
static symtblbucket* getsymtabentry( stryng,identtype );
static edgelisthead* addnametolist( edgelisthead**,stryng );
static edgelisthead* addtoklist( stryng,port*,edgelisthead* );
static edgelisthead* addtoforklist( stryng,port*,treestackobj* );
static edgelisthead* addtoiklist( stryng,port*,treestackobj* );
static void addtoolist( port*,node* );
static void addtoilist( port*,node* );
static symtblbucket* copysym( symtblbucket* );
static port* emptyoldedge( symtblbucket* );
static port* emptyedge( symtblbucket* );
static int nextkport( treestackobj* );
static symtblbucket* pullname( symtblbucket*,treestackobj*,int );
static void addedge( inforecord*,inforecord* );
static void pushnilttptr( void );
static void pushbasic( int );
static void pushtypename( stryng,int,int );
static void pusharray( boolean );
static void pushstream( boolean );
static void pushrecord( boolean );
static void pushunion( boolean );
static void pushnulltype( void );
static void checktypename( stryng,int,int );
static void finishtypelinks( void );
static void linkbase( void );
static void buildfort( char,boolean );
static void buildfield( boolean );
static void buildtag( boolean );
...
static inforecord* createerr( stentry** );
static void endsbuild( int,int );
static void beforeexp( void );
static void incboostfac( void );
static void decboostfac( void );
static void buildunarynode( explistnode*,opertype,int );
static void buildbinarynode( explistnode*,explistnode*,opertype,int );
static boolean Reduce( void );
static void reduceexp( void );
static void afterexp( void );
static void aftersimpexp( void );
static void dobinaryop( int,int,int );
static void poppreclevel( void );
static void findfieldname( stryng,stentry**,boolean*,int* );
static inforecord* createerrlit( void );
static void dofieldlist( int,int );
static void startrbuild( void );
static void buildfieldlist( int,int );
static void endrbuild( int,int );
static void pushniltag( void );
static void builderrunion( semanticrec*,semanticrec*,semanticrec** );
static void endubuild( int,int );
static void pushinfolist( ilistrec**,inforecord* );
static inforecord* popinfolist( ilistrec** );
static void pushpositionlist( plistrec**,int );
static int poppositionlist( plistrec** );
static void buildreplace( int,int );
static void popsymlist( void );
static void startsymlist( void );
static void checkatypes( int,int );
static void checkarity( void );
static void assoctypes( void );
static void errindexnames( namelistrec**,int );
...
static void checkexptype( explistnode**,int,int );
static void makearrayfor( namelistrec*,explistnode*,namelistrec*,int,int );
static void makerangefor( explistnode*,explistnode*,namelistrec*,int,int );
static void makeforall( int,int );
static void increasegenerator( int,int );
static void explicitcross( int,int );
static void endforall( void );
static edgelisthead* findnameonlist( stryng,edgelisthead* );
static void endinitpart( void );
static void endabody( int,int );
static void endbbody( int,int );
...
static void checkmask( explistnode* );
static void dounlessexp( void );
static void dowhenexp( void );
static void nomaskingexp( void );
static inforecord* removelast( inforecord*,treestackobj* );
static stentry* buildredtype( stentry* );
static void switchtoreturngraph( treestackobj* );
static void switchtobodygraph( treestackobj* );
static void dovalueof( void );
static void doarrayof( int );
static void startreturns( void );
static void endforinit( void );
static void notenoughargs( stryng,struct LOC_dopredefined* );
static void toomanyargs( stryng,struct LOC_dopredefined* );
static void wrongargtype( int,stryng,struct LOC_dopredefined* );
static inforecord* getalit1_( struct LOC_dopredefined* );
static void checknumberofargs( semanticrec*,int,stryng,struct LOC_dopredefined* );
static inforecord* addintnode( inforecord*,struct LOC_dopredefined* );
static inforecord* buildpredefnode( int,int,struct LOC_dopredefined* );
static inforecord* errtype_( char,stentry*,struct LOC_dopredefined* );
static inforecord* maxmin( unchar,semanticrec*,int,stryng,struct LOC_dopredefined* );
static inforecord* addhlarrayorstream( char,semanticrec*,unchar,int,stryng,struct LOC_dopredefined* );
static inforecord* sizearrayorstream( predeftype,char,semanticrec*,int,stryng,struct LOC_dopredefined* );
static inforecord* remhlarrayorstream( char,semanticrec*,unchar,int,stryng,struct LOC_dopredefined* );
static inforecord* arraylimhorl( unchar,semanticrec*,int,stryng,struct LOC_dopredefined* );
static inforecord* floorortrunc( unchar,semanticrec*,int,stryng,struct LOC_dopredefined* );
static void dopredefined( symtblbucket*,semanticrec*,int );
static void initsem( void );
static boolean linkdefinename( stryng,namelistrec** );
static void addtodefinelist( stryng );
static void addtotypelist( void );
static void starttypelist( void );
static stryng buildfunctname( stryng );
static void addlkptr( linkrec* );
static void addgrptr_( graph* );
static void buildfunctgraph( symtblbucket*,stryng,int,boolean );
static boolean checkdefinelist( stryng,int );
static void startfunction( stryng,int,int,int );
static void enddefinelist( void );
static void startforwardfunct( stryng,int,int );
static void addfunctargtype( stentry*,stentry* );
static void processparam( void );
static void addfunctrestype( stentry*,stentry* );
static void doresultlist( typelistrec*,symtblbucket* );
static boolean checklastlevel( stryng,identtype,symtblbucket** );
static void freelist( stentry* );
static void freefuncttype( stentry* );
...
static void startif( lexleveltype );
static void startifsubgraph( void );
static void endiftagsubgraph( boolean );
...
static void createsub( node*,tagflag );
static void changeto2( assoclist* );
static void createisunion( int,int );
static void checkold( stryng,int,int );
static void pushuplus( int,int );
static void pushuminus( int,int );
static void pushunot( int,int );
static void createiserror( int,int );
static void fixforfunct( void );
static stentry* errorstruct( void );
static void fixfortag( int,int );
static void processtaghead( int,int );
static void createtagsubgraph( void );
static tcstacklistrec* findtag( stryng,tcstacklistrec* );
static void checktagnames( int,int );
static void assocexprs( void );
static void endtagcase( void );
static void otherwisetag( int,int );
static void setstep( void );
static void showbrkpts( void );
static void removebrkpts( void );
static void addbrkpts( void );
static void printnames( namelistrec* );
static void printtypes_( typelistrec* );
static void printgraphinfo( inforecord* );
static void printsymlist( symlistnode* );
static void printexplist( explistnode* );
static void printtclist( tcstacklistrec* );
static void printstack( void );

static void readcosts( void );
static void reads( void );
static void checkinsertsize( int );
static void get1prefix( void );
static void skipcosts( void );
static void skips( void );
static void skip1prefix( void );
static void seeke( char,stryng );
static void getprefixinfo( char,stryng );
static void readerrtables( stryng* );
static void copye( SHORT,stagerec* );
static void copys( SHORT,stagerec* );
static void printinsertion( stagerec );
static void expandinsertion( SHORT,stagerec* );
static void llinsert( char,stagerec*,stryng* );
static void inoverflow( void );
static void llcorrector( SHORT*,stryng* );
static void myabort( void );
static void initptab( void );
static SHORT findlltab( SHORT,char );
static void printstring( stryng );
static void printsym( int );
static void printline( inputbuffer* );
static void modify( inputbuffer* );
static void flushlines( void );
static void readline( inputbuffer* );
static void sisal_getline( inputbuffer** );
static void readchar( Char*,inputbuffer** );
static void unreadchar( Char,inputbuffer* );
static void insertcharbuf( Char,unchar,inputbuffer* );
static void displaydeletions( int );
static void displayinsertion( SHORT );
static void scanerror( void );
static SHORT searchsymboltable( int,int,stryng*,SHORT );
static SHORT lookupterm( stryng*,SHORT,struct LOC_scan* );
static void ctrlchtooctal( Char,stryng*,struct LOC_scan* );
static int branchonfirstchar( struct LOC_scan* );
static void scannererr( errtype,struct LOC_scan* );
static void unappend( stryng*,inputbuffer**,struct LOC_scan* );
static void scan( SHORT*,int*,inputbuffer** );
static void initscanner( void );
static void peek( SHORT* );
static void gettok( SHORT*,int* );
static void deletetokens( int );
static void inserttokens( stagerec );
static void checkerrorok( void );
static void readtables( stryng );
static void push( SHORT );
static int tos( void );
static void pop( void );
static void trace( void );
static boolean checkepsilonok( char );
static void parseerror( SHORT,stryng* );
static void printstats( void );
static void init( void );
static void parse( stryng* );
static void readline_( int,inputbuffer*,FILE*,struct LOC_pass1* );
static boolean recursiveincl( stryng,struct LOC_pass1* );
static void pushinclstack( stryng,int,struct LOC_pass1* );
static void popinclstack( struct LOC_pass1* );
static void dumpline( inputbuffer*,struct LOC_pass1* );
static char* CANN_StringCopy(int,char*);
static int CANN_PreparePragmaName( char* );
static int CANN_IsEntryPragma( char* );
static int CANN_IsFortranPragma( char* );
static int CANN_IsCPragma( char* );
static void CANN_ProcessPragmaFields( int,char*,char**,int* );
static int CANN_DissectSpecialPragmas( int,int,char* );
static char* CANN_StripWhites( char* );
static void error( struct LOC_dissectpragma* );
static void dissectpragma( inputbuffer*,int,int,pragmaname*,stryng*,struct LOC_pass1* );
static boolean pass1( stryng*,stryng*,FILE* );
static void splitaelement( node* );
static void convertareplacen( node* );
static void removeunusedports( node* );
static node* createselectorgraph( node* );
static void convertiftoselect( node* );
static void convertelseiftoselect( node* );
static void searchgraph_( node* );
static void converttoversion1( void );
static boolean nodesareequivalent( node*,node*,struct LOC_removegraphcse* );
static void inittable( struct LOC_removegraphcse* );
static void addtotable_( unchar,node*,struct LOC_removegraphcse* );
static node* getequivnode( unchar,node*,struct LOC_removegraphcse* );
static void movealloutputedges( node*,node*,struct LOC_removegraphcse* );
static void moveoutputedges( node*,int,int );
static void pushfanoutout( node*,struct LOC_removegraphcse* );
static int removegraphcse( node*,boolean );
static ndptrreclist* ndptrnewlist( void );
static boolean ndptrisnull( ndptrreclist* );
static node* ndptrrmhead( ndptrreclist* );
static int ndptrlength( ndptrreclist* );
static ndptrreclist* ndptrappend( ndptrreclist*,node* );
static void imposegraphordering( ndptrreclist*,node* );
static int numancestors( node* );
static void setrefcount( node*,int );
static int refcount( node* );
static boolean dfordergraph( node* );
static boolean allconstantinputs( node*,struct LOC_fixreturnsgraph* );
static void createnewkport( struct LOC_fixreturnsgraph* );
static void createnewlport( struct LOC_fixreturnsgraph* );
static void insureedgeisnotconstant( node*,int,struct LOC_fixreturnsgraph* );
static void moveoutsideofloop( node*,struct LOC_fixreturnsgraph* );
static void copynodetograph( node*,node*,int,struct LOC_fixreturnsgraph* );
static void movenodeafterloop( node*,node*,node*,struct LOC_fixreturnsgraph* );
static void removefinalvalue( node*,node*,struct LOC_fixreturnsgraph* );
static void fixreturnsgraph( node* );
static void findloopnodes( node* );
static boolean dforder( node* );
...
static treestackobj *currentlevel;
static int linenumber, tokenlineno;

static void deletestring(stryng*,int,int);
static void dumpmodule(void);
static boolean equalstrings(stryng*,stryng*);
static void insertchar(stryng*,Char,int);
static void mymemcpy(char*,char*,int);
static graph* newgraphptr(void);
static void newnamelist(namelistrec**);
static void removenode(node*);
static void string10(stryng*,Char*);
static void string20(stryng*,Char*);
static Char stringchar(stryng*,int);
static void stringlowercase(stryng*);
static void substring(stryng*,stryng*,int,int);
static void writestring(FILE*,stryng*);
static PSBLOCK MySBlockAlloc(void);
static PBBLOCK MyBBlockAlloc(void);
static void addtoexplist(explistnode**,explistnode*);
static void addtonamelist(stryng,int,int);
static stentry *addtotypetable(stentry*);
static void adjustgraphptrs(void);
static boolean announceaction(int,int,int,int,stryng);
static void askordefault(parrec*,stryng*,boolean,Char*);
static int assignports(edgelisthead*,int);
static boolean canwrite(stryng*);
static void changeedgedest(port*,node*,int);
static void changeedgesrc(port*,node*,int);
static void compactports(node*);
static void compacttypetable(int*,int);
static void concatchar(stryng*,Char);
static node *copynode(node*);
static node *copygraph(node*);
static node *copycompound(node*);
static node *createcompoundnode(int);
static node *createsimplenode(int);
static boolean debugger(int,int);
static void defaultext(stryng*,stryng*,Char*);
static parrec *defineparameter(parrec**,Char*,Char*,partyp,int,int);
static void disconnectedgefromdest(port*);
static void disconnectedgefromsource(port*);
static int findstring(stryng*,stryng*,int);
static void freeinfoptr(inforecord**);
static void freenamelist(namelistrec**);
static void freesemantic(semanticrec**);
static void freesymptr(symtblbucket*);
static void freetornlist(tornrec**);
static void freetreestack(treestackobj*);
static void freettptr(stentry**);
static void freetypelist(typelistrec**);
static inforecord *getalit1(void);
static port *getinputedge(node*,int);
static port *getoutputedge(node*,int);
static node *graphwalk(node*);
static void initmodule(void);
static void initnames(void);
static port *insertedge(node*,int,node*,int,stentry*,stryng);
static port *insertliteral(node*,int,stentry*,stryng,stryng);
static void insertnode(node*,node*,node*);
static void integerstring(stryng*,int,int);
static int largestinputportnumber(node*);
static int largestoutputportnumber(node*);
static void linkparent(node*,treestackobj*);
static tornrec *linktorntype(tornrec*,tornrec*);
static void linktype(stentry*,typelistrec**);
static edgelist *newedgelistptr(void);
static port *newedgeptr(portsort);
static errorrecord *newerrorptr(errortypes);
static explistnode *newexplistptr(void);
static edgelisthead *newheadptr(void);
static inforecord *newinfoptr(void);
static linkrec *newlinkageptr(linksort);
static node *newnodealloc(nodesort);
static node *newnodeptr(nodesort,treestackobj*);
static refrec *newreferencesptr(void);
static void newsemantic(semanticrec**,typesemanticdata);
static symlistnode *newsymlistptr(void);
static tcstacklistrec *newtcstacklist(void);
static void newtornlist(tornrec**,torntype);
static void newtypelist(typelistrec**);
static void nextbodyportname(treestackobj*,stryng*);
static port *nextoutputedgesameport(port*);
static int nextreturnport(treestackobj*);
static int numberofwiredoutputports(node*);
static int numbnodesingraph(node*);
static int numbofsubgraphs(node*);
static boolean openintread(FILE**,stryng*);
static boolean openlistfile(stryng,FILE**);
static boolean openread(FILE**,stryng*);
static boolean openwrite(FILE**,stryng*);
static port *outputedgegeport(node*,int);
static semanticrec *popsemantic(void);
static node *producernodeofedge(port*);
static int producerportnumber(port*);
static void pushemptyexplist(void);
static void pushsemantic(semanticrec*);
static void putintable(stentry**);
static void readcommandline(parrec**,parrec**,parrec**);
static void removedeadcode(void);
static void removeedge(port**);
static void semerror(errorrecord*);
static void shiftinputports(node*,int,int);
static void shiftoutputports(node*,int,int);
static void smashtypes(void);
static void splitname(stryng*,stryng*,stryng*,stryng*);
static void startnamelist(void);
static comparetype stringcompare(stryng*,stryng*,int);
static long stringnumber(stryng*,int*,int);
static void stringuppercase(stryng*);
static node *tobodygraph(node*);
static node *tochildgraph(node*,int);
static node *toforallbodygraph(node*);
static node *toforallgeneratorgraph(node*);
static node *toforallreturnsgraph(node*);
static node *toinitgraph(node*);
static node *tolastnodeingraph(node*);
static node *tonextfunction(node*);
static node *toprevnode(node*);
static node *toreturnsgraph(node*);
static node *totestgraph(node*);
static void insertstring(stryng*,stryng*,int);
static void stripspaces(stryng*);
static void cleanupgraph(node*);
static void CANN_DumpSpecialPragmas(FILE*);
...
static int boostfac;
static boolean cortrace;
static boolean debug;
static long arithtypes;
static long algebtypes;
...
static int numterms;
static FILE* ptableout;
static FILE* etableout;
...
static stryng listfilename;
static stryng stampstr;
static stryng versionstr;
static stryng versionnum;
static int myinfinity;
static char curerrsym;
static boolean errortables;
static SHORT stackptr;
static int savinsertptr;
static int errorcount;
static int numdeltried;
static int tokinserted;
static int tokdeleted;
static parrec *directoryparam;
static parrec* paramlist;
static int semerrcnt;
static parrec *xrefparam;
static parrec* listingparam;
static parrec* debugparam;
static parrec* pass1param;
static boolean passerr;
static stryng directoryname;
static stryng defaultdirectoryname;
static stryng filename;
static parrec *listfparam;
static boolean listing;
static namelistrec *includelist;
static int stringptr;
static inputbuffer* linebuf;
static inputbuffer* oldline;
static boolean echoed;
static boolean endfile;
static stryng curinputfile;
static stryng newfname;
static unchar tokenstart;
static boolean debugcg;
static boolean commentflag;
static boolean peeking;
static boolean firsttime;
static boolean firsttime;
static int newlinenum;
static SHORT oldtoken;
static int toconsume;
...
static int CANN_GlueMode;
static tablerec p;
static boolean done;
static int tokenposition;   /* character position within a token */
static void clearstring( stryng*,int,int );
static void readstring( FILE*,stryng* );
static Char uppercase( Char );
static Char lowercase( Char );
static boolean matchstrings( stryng*,stryng*,int );
static int findchar( Char,stryng*,int );
static int findlastchar( Char,stryng* );
static void numberstring( stryng*,int,int );
static int charval( Char );
static void initnames( void );
static void buildname( stryng*,stryng,stryng,stryng );
static void splitname( stryng*,stryng*,stryng*,stryng* );
static void defaultfilename( stryng*,stryng* );
static boolean openread( FILE**,stryng* );
static boolean openintread( FILE**,stryng* );
static boolean openwrite( FILE**,stryng* );
static boolean canwrite( stryng* );
static void gettermline( stryng*,stryng* );
static void getcommandline( stryng* );
static void CANN_exit( int );
static parrec* defineparameter( parrec**,Char*,Char*,partyp,int,int );
static void setparameter( parrec*,stryng* );
static Char gettoken( stryng*,stryng* );
static boolean parsecommandline( parrec* );
static void getparamvalue( parrec*,stryng );
static void defaultext( stryng*,stryng*,Char* );
static void readcommandline( parrec**,parrec**,parrec** );
static boolean openoutputfile( stryng*,FILE** );
static stentry* newtypealloc( char );
static port* newedgealloc( portsort );
static node* newnodealloc( nodesort );
static void initstamps( void );
static void removestamp( Char );
static void addstamp( Char,stryng );
static int numbofsubgraphs( node* );
static int numbnodesingraph( node* );
static int numberofwiredoutputports( node* );
static int largestinputportnumber( node* );
static int largestoutputportnumber( node* );
static int nodelabel( node* );
static node* producernodeofedge( port* );
static int producerportnumber( port* );
static node* tolastnodeingraph( node* );
static node* toprevnode( node* );
static node* tochildgraph( node*,int );
static node* toenclosingcompound( node* );
static node* toenclosingfunction( node* );
static node* toinitgraph( node* );
static node* tobodygraph( node* );
static node* totestgraph( node* );
static node* toreturnsgraph( node* );
static node* toiterbodygraph( node* );
static node* toselectorgraph( node* );
static node* toforallgeneratorgraph( node* );
static node* toforallbodygraph( node* );
static node* toforallreturnsgraph( node* );
static port* getinputedge( node*,int );
...
static port* nextoutputedgesameport( port* );
static port* outputedgegeport( node*,int );
static node* getnodewithlabel( node*,int );
static node* getnodewithid( node*,int );
static node* tonextfunction( node* );
static stentry* searchtypetable( char,stentry*,stentry*,stryng );
static int equivclass( stentry*,struct LOC_smashtypes* );
static boolean sameequivclass( stentry*,stentry*,struct LOC_smashtypes* );
static void createnewequivclass( stentry*,struct LOC_smashtypes* );
static void removefromequivclass( stentry*,stentry*,struct LOC_smashtypes* );
static void addtoequivclass( stentry*,stentry*,struct LOC_smashtypes* );
static void initequivclasses( struct LOC_smashtypes* );
static void gatherothers( stentry*,stentry**,struct LOC_smashtypes* );
static void pointtohead( struct LOC_smashtypes* );
static void smashtypes( void );
static void adjustgraphptrs( void );
static stentry* getbasictype( char );
static stentry* getwildtype( void );
static stentry* getconstructortype( char,stentry* );
static void putintable( stentry** );
static void putstruct( stentry* );
static stentry* addtotypetable( stentry* );
static ifgraphtype graphkind( node* );
static void disconnectedgefromsource( port* );
static void disconnectedgefromdest( port* );
static void removeedge( port** );
static void removeinputedges( node* );
static void removegraph( node* );
static void removenode( node* );
static node* createsimplenode( int );
static node* createcompoundnode( int );
static void insertnode( node*,node*,node* );
...
static port* insertedge( node*,int,node*,int,stentry*,stryng );
static port* insertliteral( node*,int,stentry*,stryng,stryng );
static void renumbergraph( node*,boolean );
static void shiftinputports( node*,int,int );
static void shiftoutputports( node*,int,int );
static node* copynode( node* );
static node* copycompound( node* );
static void copyedges( node*,node* );
static node* copygraph( node* );
static void cleanupnewiter( node* );
static void cleanupforall( node* );
static void cleanupiterloop( node* );
static void cleanupselect( node* );
static void cleanuptagcase( node* );
static void cleanupifthenelse( node* );
static void nsinit( ndstack* );
static boolean nsempty( ndstack* );
static void nspush( ndstack*,node* );
static node* nspop( ndstack* );
static void cleanupgraph( node* );
static void removedeadcode( void );
static void rm( node* );
static void removenoop( node* );
static int compactinputs( node*,int,int );
static void compactports( node* );
static void buildbasictype( char,Char* );
static void initmodule( void );
static void writetypenumber( stentry*,struct LOC_dumpprogram* );
static void dumpentry( stentry*,struct LOC_dumpprogram* );
static void writeedgepragmas( port*,struct LOC_dumpprogram* );
static void dumpinputedges( node*,struct LOC_dumpprogram* );
static void writenodepragmas( node*,struct LOC_dumpprogram* );
static void dumpsimple( node*,struct LOC_dumpprogram* );
static void dumpcompound( node*,struct LOC_dumpprogram* );
static void dumpgraph( node*,struct LOC_dumpprogram* );
static void dumpif1file( FILE*,struct LOC_dumpprogram* );
static void dumpprogram( stryng );
static void dumpmodule( void );
static char hashfunction( Char*,struct LOC_graphwalk* );
static boolean emptylist( intlist*,struct LOC_graphwalk* );
static void initlist( intlist*,struct LOC_graphwalk* );
static int poplist( intlist*,struct LOC_graphwalk* );
static void addtolist( intlist*,int,struct LOC_graphwalk* );
static void displaytable( struct LOC_graphwalk* );
static void insert_( Char*,commandtype,struct LOC_initcommandtable* );
static void initcommandtable( struct LOC_graphwalk* );
static void skipblanks( struct LOC_graphwalk* );
static boolean readinteger_( int*,struct LOC_graphwalk* );
static void readcommandname( Char*,struct LOC_getvalidcommand* );
static boolean foundintable( Char*,commandtype*,struct LOC_getvalidcommand* );
static void getvalidcommand( commandtype*,struct LOC_graphwalk* );
static int findlooplevel( node*,struct LOC_graphwalk* );
static int findlexicallevel( node*,struct LOC_graphwalk* );
static void displayedge( port*,struct LOC_graphwalk* );
static void displaynode( node*,struct LOC_graphwalk* );
static void writetype( stentry* );
static void displaytype( struct LOC_graphwalk* );
static void marknode( node*,struct LOC_graphwalk* );
static boolean nodemarked( node*,struct LOC_graphwalk* );
static void unmarkgraph( node*,int,struct LOC_graphwalk* );
static void gatherboundaryedges( intlist*,node*,int*,struct LOC_graphwalk* );
static void listallfuns( struct LOC_graphwalk* );
static void listallnodes( node*,struct LOC_graphwalk* );
static void gotonextnode( node**,struct LOC_graphwalk* );
static void gotoparent( node**,struct LOC_graphwalk* );
static void showcurrfun( node*,struct LOC_graphwalk* );
static void gotonodeid( node**,struct LOC_graphwalk* );
static void showoutedges( node*,struct LOC_graphwalk* );
static void showinedges( node*,struct LOC_graphwalk* );
static void gotosubgraph( node**,struct LOC_graphwalk* );
static void gotonodelabel( node**,struct LOC_graphwalk* );
static void searchgraph( node*,int,struct LOC_findnodes* );
static void findnodes( node*,boolean,struct LOC_graphwalk* );
static void moveright_( node**,struct LOC_graphwalk* );
static void moveleft_( node**,struct LOC_graphwalk* );
static void shownodesabove( node*,struct LOC_graphwalk* );
static void shownodesbelow( node*,struct LOC_graphwalk* );
static void showgraph( node*,struct LOC_graphwalk* );
static void givehelp( struct LOC_graphwalk* );
static void showgraphports( node*,struct LOC_graphwalk* );
static void dumpnode( node*,struct LOC_graphwalk* );
static void dumponeedge( port*,struct LOC_graphwalk* );
static void dumpdepinputedges( node*,struct LOC_graphwalk* );
static void dumpdepoutputedges( node*,struct LOC_graphwalk* );
static void dumpinputedges_( node*,struct LOC_graphwalk* );
static void dumpallinput( node*,struct LOC_graphwalk* );
static void dumpoutputedges( node*,struct LOC_graphwalk* );
static void dumpalloutput( node*,struct LOC_graphwalk* );
static node* graphwalk( node* );
static int nextnodenum( void );
static int nextedgenum( void );
static int nextsgnodenum( treestackobj* );
static int nextreturnport( treestackobj* );
static void nextbodyportname( treestackobj*,stryng* );
static void freesymptr( symtblbucket* );
static void freetreestack( treestackobj* );
...
static void newtornlist( tornrec**,torntype );
static void freetornlist( tornrec** );
static void newttptr( stentry**,char );
static void freettptr( stentry** );
static node* newnodeptr( nodesort,treestackobj* );
static port* newedgeptr( portsort );
static linkrec* newlinkageptr( linksort );
static graph* newgraphptr( void );
static edgelisthead* newheadptr( void );
static void freeheadptr( edgelisthead** );
static edgelist* newedgelistptr( void );
static void freeedgelistptr( edgelist** );
static refrec* newreferencesptr( void );
static inforecord* newinfoptr( void );
static void freeinfoptr( inforecord** );
static explistnode* newexplistptr( void );
static tcstacklistrec* newtcstacklist( void );
static errorrecord* newerrorptr( errortypes );
static void freeerrorptr( errorrecord** );
static symlistnode* newsymlistptr( void );
static void printtypesort( FILE*,stentry* );
static void printoperation( FILE*,opertype );
static void printerror( FILE*,errorrecord* );
static void semerror( errorrecord* );
static node* findparent( treestackobj* );
static void linkparent( node*,treestackobj* );
static void pushsemantic( semanticrec* );
static semanticrec* popsemantic( void );
static void linkname( stryng,int,int,namelistrec** );
static tornrec* linktorntype( tornrec*,tornrec* );
static void linktype( stentry*,typelistrec** );
static void addtonamelist( stryng,int,int );
static void startnamelist( void );
...
static void newsymlist( symlistobj**,struct LOC_printtable* );
...
static void freesymlist( symlistobj*,struct LOC_printtable* );
static void freesymlist(symlistobj *sym, struct LOC_printtable *LINK)
{
  /* Free(sym); */
}  /* FreeSymList */

static void addtofunctlist( symtblbucket*,struct LOC_printtable* );
